...
# Configurazione
NUM_EPISODES = 150
SAVE_MODEL_PATH = os.path.join("saved_models", "dicewars_rl_model.keras")
...
other_players = [AgressivePlayer(), RandomPlayer(), RandomPlayer()]
...
for episode in tqdm(range(NUM_EPISODES), desc="Episode"):
    ...
    epsilon = max(0.01, 0.7 * (0.95 ** episode))  # Decrescente
    ...
    while match.winner == -1:
        ...
        if current_player == 0:
            ...
            if done:
                ...
                # for i in range(1, min(last_n + 1, len(buffer)) + 1):
                # # Unpack the tuple
                #     state_vec, action_idx, action, reward, next_state_vec, done = buffer[-i]

                #     new_reward = reward + bonus

                #     # Replace the tuple in the buffer
                #     buffer[-i] = (state_vec, action_idx, action, new_reward, next_state_vec, done)
