...
#qua
# use os.gtcwd for relative path
...
class RLDicewarsAgent:
    ...
    def encode_state(self, grid, match_state):  # da rivedere, usare descrittori
        flat_dice = np.array(match_state.area_num_dice) / 8.0   # 8 max number of dice per area
    ...
    def encode_state_new(self, grid, match_state):  # da rivedere, usare descrittori
        flat_dice = np.array(match_state.area_num_dice) / 8.0   # 8 max number of dice per area
        flat_owner = np.array(match_state.area_players) / 3.0   
        state_vec = np.concatenate([flat_dice, flat_owner])
    ...
    def build_action_mapping(self, grid):
        #num_areas = len(grid.areas)
        #print("Numero reale di aree:", num_areas)

        max_areas = Grid.DEFAULT_MAX_NUM_AREAS
